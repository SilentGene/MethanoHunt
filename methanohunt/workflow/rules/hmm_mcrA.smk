...
rule hmmsearch_mcrA_N:
    ...
    output:
        tbl = f"{config['output_dir']}/cust_db_anno/hmm/{{genome}}_McrA_N.tbl"
    threads: 4
    shell:
...
rule hmmsearch_mcrA_C:
    ...
    output:
        tbl = f"{config['output_dir']}/cust_db_anno/hmm/{{genome}}_McrA_C.tbl"
    threads: 4
    shell:
        "hmmsearch --cpu {threads} --cut_tc --tblout {output.tbl} {input.hmm} {input.faa} > /dev/null"
...
rule merge_mcrA_hits:
    input:
        n = f"{config['output_dir']}/cust_db_anno/hmm/{{genome}}_McrA_N.tbl",
        c = f"{config['output_dir']}/cust_db_anno/hmm/{{genome}}_McrA_C.tbl"
    output:
        tbl = f"{config['output_dir']}/cust_db_anno/hmm/{{genome}}_McrA.tbl"
    shell:
        """
...
rule extract_hits_mcrA:
    input:
        ...
        tbl = f"{config['output_dir']}/cust_db_anno/hmm/{{genome}}_McrA.tbl"
    output:
        faa = f"{config['output_dir']}/cust_db_anno/hits/{{genome}}_McrA_hit.faa"
    script:
